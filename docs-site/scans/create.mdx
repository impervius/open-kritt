...
| `model` | Yes | Provider model ID or supported alias. DeepSeek requires a current catalog entry; OpenRouter and xAI also accept an exact custom model ID. The Custom provider always takes an exact model ID served by its endpoint. |
| `model_provider` | Yes | `codex`, `claude`, `custom`, `deepseek`, `openrouter`, or `xai`. |
...
`custom` + `codex`, `deepseek` + `codex`, `openrouter` + `claude-code` or advanced `codex`, and
